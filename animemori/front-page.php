...
$genre_like = $genre ? '%"name":"' . $wpdb->esc_like($genre) . '%': '';
$genre_sql = $genre ? " AND source_payload_json LIKE %s" : '';

// Build genre list for filter
...
if ($genre) {
  $next_season = $wpdb->get_results($wpdb->prepare(
    "SELECT id, title_english, title_romaji, slug, cover_image_url, year, start_date, status
     FROM {$anime_table}
  ...
  $popular = $wpdb->get_results($wpdb->prepare(
    "SELECT a.id, a.title_english, a.title_romaji, a.slug, a.cover_image_url, a.year,
     ...
     WHERE a.status IN ('AIRING','UPCOMING','FINISHED')
       AND a.source_payload_json LIKE %s
     GROUP BY a.id
    ...
    $wpdb->prepare(
      "SELECT id, title_english, title_romaji, slug, cover_image_url, year, start_date
       ...
       WHERE ((start_date IS NOT NULL AND YEAR(start_date) = %d)
         ...
         AND source_payload_json LIKE %s
       ORDER BY start_date ASC, id DESC
       LIMIT 24",
      $next_year,
      $next_year,
...
$featured_year = ($featured && $featured->year) ? $featured->year : '';
  ...
  <div class="container">
    <div class="hero-inner">
      <div class="hero-content">
        ...
        <p class="hero-sub">
          <?php echo $featured_year ? 'Premiered ' . esc_html($featured_year) . '. ' : ''; ?>
          Weekly anime schedule and upcoming releases in one place.
        </p>
        <div class="hero-actions">
          <a class="btn" href="<?php echo esc_url(home_url('/schedule/')); ?>">View schedule</a>
          <a class="btn ghost" href="<?php echo esc_url(home_url('/upcoming/')); ?>">Upcoming releases</a>
        </div>
      </div>
      <div class="hero-art">
        <img src="<?php echo esc_url(get_template_directory_uri() . '/assets/logo-mascot.png'); ?>" alt="AnimeMori mascot" loading="eager" fetchpriority="high" decoding="async">
      </div>
    </div>
  </div>
</section>
    ...
    <div>
      <form class="filters" method="get" action="<?php echo esc_url(home_url('/')); ?>">
        <div class="filter-field">
          ...
          <select id="home-genre" name="genre">
        ...
        <div class="filter-actions">
          ...
          <a class="btn ghost" href="<?php echo esc_url(home_url('/')); ?>">Reset</a>
        </div>
      </form>
